...
int main()
{
	...
	strcat(readfilename1, "fly_Jonny_mesures_20161118_204012_accZ.csv"); //Gyros
	strcpy(readfilename2, filepath);
	strcat(readfilename2, "fly_Jonny_mesures_20161118_204012_timeInterval.csv");//*/

	strcpy(savefilename, "center_downSample500_CMean_fly_Jonny_mesures_20161118_204012_1000accZ");
	...
	while (T--)
	{
		...
		if (sumtime == 0 && (downsamplecount % downsamplenum == 0)) fout << Totaltime[0] << " " << oridata[0] << endl;
		else
		{
			pre = sumdata / sumtime;
		...
		for (int i = 1; i < oridata.size(); i++)
		{
			...
			if (sumtime == 0 && (downsamplecount % downsamplenum == 0))
			{
				fout << Totaltime[i] << " " << pre << endl; cout << "invalid" << endl;
			}
			else if (downsamplecount % downsamplenum == 0)
			{
				pre = sumdata / sumtime;
