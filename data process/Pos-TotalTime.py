...
filepath = 'F:/实验结果/新实验结果8_60s/速度均值位移均值为零/'
#filepath = 'F:/实验结果/新实验结果10_120s/加速度均值位移均值为零/'
filepath=unicode(filepath,'utf8')
f1 = open(filepath+'TotalTime.csv','rb')
f2 = open(filepath+'Value_X-Position.csv','rb')
f3 = open(filepath+'Value_Y-Position.csv','rb')
f4 = open(filepath+'Value_Z-Position.csv','rb')
...
np.savetxt(savePath+'Value_X-Position.txt',np.hstack((totaltime,dataX)),fmt='%f',delimiter=' ')
np.savetxt(savePath+'Value_Y-Position.txt',np.hstack((totaltime,dataY)),fmt='%f',delimiter=' ')
np.savetxt(savePath+'Value_Z-Position.txt',np.hstack((totaltime,dataZ)),fmt='%f',delimiter=' ')