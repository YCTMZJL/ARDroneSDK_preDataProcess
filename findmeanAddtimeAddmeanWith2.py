...
f1 = open('/home/ml/Software/ARDrone_SDK_2_0_1/Examples/Linux/Build/Release/mesures_y30_2_20160901_102458.txt','rb')
lines=f1.readlines();
f1.close()
count = 0
indexRecord = []
...
#meanaccY =0#n
meanaccY = np.mean(data[1:10,1].T)# 33.159057149
#67.7608625058#70.472140655##69.4756070888
#meanaccZ = -1000#
meanaccZ = np.mean(data[1:10,2].T)#-1031.950#-1028.0009666
...
print  'accX_cur = ',np.mean(data[1:,0].T),' accY_cur = ',np.mean(data[1:,1].T),' accZ_cur = ',np.mean(data[1:,2].T)
print  'accX = ',np.mean(data[1:10,0].T),' accY = ',np.mean(data[1:10,1].T),' accZ = ',np.mean(data[1:10,2].T)
...
plt.plot(range(len(data[:,0].T)), vel[:,0].T,'b.-',label='accX') #[:,8:11]
plt.plot(range(len(data[:,1].T)), vel[:,1].T,'r.-',label='accY')
plt.plot(range(len(data[:,2].T)), vel[:,2].T,'g.-',label='accZ')
#########################################################
plt.figure('sZ')
